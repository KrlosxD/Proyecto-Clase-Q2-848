using System.Security.Cryptography;
using System.Text;
using Google.Cloud.Firestore;
using ProyectoClase.DTOs;
using ProyectoClase.Models;

namespace ProyectoClase.Services;

//En esta clase se maneja lo relaciona la registro
//e inicio de sesión
public class AuthService
{
    private readonly FirebaseService _firebaseService;
    private readonly IConfiguration _cofiguration;
    
public AuthService(FirebaseService firebaseService, IConfiguration cofiguration)
    {
        _firebaseService = firebaseService;
        _cofiguration = cofiguration;
    }

    public async Task<User> Register(RegisterDto dto)
    {
        //Primero verificar que no exista un usuario con este correo
        var collection = _firebaseService.GetCollection("users");
        var existing = await collection
            .WhereEqualTo("Email", dto.Email)
            .GetSnapshotAsync();

        if (existing.Count > 0)
            throw new Exception("Ya existe un usuario con ese correo");

        //Darle permiso para que se pueda crear el nuevo usuario
        var user = new User
        {
            Id = Guid.NewGuid().ToString(),
            FullName = dto.FullName,
            Email = dto.Email,
            Password = HashPassword(dto.Password),
            Role = "user",
            CreatedAt = DateTime.UtcNow
        };

        //Guardamos el objeto en FireStore
        await collection.Document(user.Id).SetAsync(new Dictionary<string, object>
        {
            { "Id", user.Id },
            { "FullName", user.FullName },
            { "Email", user.Email },
            { "Password", user.Password },
            { "Role", user.Role },
            { "CreatedAt", user.CreatedAt },
        });
        return user;
    }

    private string HashPassword(string password)
        {
            var bytes = SHA256.HashData(Encoding.UTF8.GetBytes(password));
            return Convert.ToBase64String(bytes);
        }

}